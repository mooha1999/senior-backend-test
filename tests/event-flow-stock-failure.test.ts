...
process.env.STOCK_SUCCESS_RATE = '0';
...
describe('Order event flow — stock failure', () => {
  let testApp: TestApp;
  ...
  it('order transitions to STOCK_FAILED when stock check fails', async () => {
    ...
    expect(orderRes.body.status).toBe('STOCK_FAILED');
  });

  it('terminal event is stock.failed', async () => {
    const terminalPromise = waitForNextTerminalEvent(testApp.eventBus);
    ...
    expect(eventName).toBe(EVENT_NAMES.STOCK_FAILED);
  });

  it('GET /orders/:id returns status STOCK_FAILED', async () => {
    const terminalPromise = waitForNextTerminalEvent(testApp.eventBus);
